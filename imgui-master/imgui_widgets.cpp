...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    auto col1 = GetColorU32(ImVec4(0.180, 0.478, 0.937, 0.400));
    auto col2 = GetColorU32(ImVec4(0.180, 0.796, 0.937, 0.400));
    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, col2, col2, col1, col1);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    auto col1 = GetColorU32(ImVec4(0.180, 0.478, 0.937, 0.400));
    auto col2 = GetColorU32(ImVec4(0.180, 0.796, 0.937, 0.400));
    window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, col2, col2, col1, col1);
    return held;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad - 1, pad - 1), check_bb.Max - ImVec2(pad - 1, pad - 1), GetColorU32(ImGuiCol_FrameBg), style.FrameRounding);
    
    if (*v) {
        auto col1 = GetColorU32(ImVec4(0.180, 0.478, 0.937, 1.000));
        auto col2 = GetColorU32(ImVec4(0.180, 0.796, 0.937, 1.000));
        window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, col2, col2, col1, col1);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (popup_open)
            ...
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Left);
        auto col1 = GetColorU32(ImVec4(0.180, 0.478, 0.937, 0.400));
        auto col2 = GetColorU32(ImVec4(0.180, 0.796, 0.937, 0.400));
        window->DrawList->AddRectFilledMultiColor(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, col2, col2, col1, col1);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    if (0.42 != 1.0f)
    {
        float shrink_amount = (float)(int)((frame_bb.Max.y - frame_bb.Min.y) * 0.5f * (1.0f - 0.42));
    ...
    auto col1 = GetColorU32(ImVec4(0.180, 0.478, 0.937, 1.000));
    auto col2 = GetColorU32(ImVec4(0.180, 0.796, 0.937, 1.000));
    window->DrawList->AddRectFilledMultiColor(draw_bb.Min, ImVec2(middle.x, draw_bb.Max.y), col2, col2, col1, col1);
    //RenderFrame(draw_bb.Min, ImVec2(middle.x, draw_bb.Max.y), frame_col, true, g.Style.FrameRounding);
    
    // render right part
    ImU32 col = GetColorU32(ImVec4(0.200, 0.200, 0.200, 1.000));
    RenderFrame(ImVec2(middle.x, draw_bb.Min.y), draw_bb.Max, col, true, g.Style.FrameRounding);
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.50f,0.50f));
